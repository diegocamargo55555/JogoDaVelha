...
public class Jogo extends Tabuleiro {
    ...
    public void iniciarJovo() {
        ...
        if ("S".equalsIgnoreCase(resposta) || "Sim".equalsIgnoreCase(resposta)) { //Continua com os jogadores e pontuações anteriores
            ...
            String name = new String(console.LerDadosString("\nQual o nome do jogador 1? ( O ) "));
            j1.setNome(name);
            jogadores.add(j1);

            name = console.LerDadosString("\nQual o nome do jogador 1? ( O ) ");
            j2.setNome(name);
            jogadores.add(j2);
    ...
    public void Jogando(Jogador j1, Jogador j2, Tabuleiro mapa) {
        ...
        while (!vencedor && !tabuleiroCompleto(mapa)) {
            ...
            if (vencedor) {
                System.out.println("Parabens " + j1.getNome() + "!"); //Se o j1 vencer é mostrada a parabenização
